...
def chatbot_message(request):
    """Enhanced chatbot endpoint with product search and recommendations.
    Supports:
    ...
    def product_list_reply(items, show_links=True):
        if not items:
            return "No products found."
        lines = []
        for p in items[:5]:  # Limit to 5 products
            ...
            if show_links:
                ...
                lines.append(f"• {title} - ₹{price} ({category})\n  View: {url}")
            else:
                lines.append(f"• {title} - ₹{price} ({category})")
        return "\n\n".join(lines)

    # Intent: greeting
    if any(k in text for k in ['hi', 'hello', 'hey', 'start']):
        reply = (
            "👋 Hello! I'm your ShopHub Assistant!\n\n"
            "I can help you:\n"
            "• Search products (e.g., 'show me phones')\n"
            "• Find cheapest items\n"
            "• Show best/premium products\n"
            "• Browse by category (Electronics, Fashion, etc.)\n"
            "• Place orders\n\n"
    ...
    # Intent: about/site
    if any(k in text for k in ['about', 'website', 'what can', 'shophub', 'help']):
        reply = (
            "🛒 ShopHub - Your one-stop online shopping destination!\n\n"
            "I can help you:\n"
            "✓ Search products by name or category\n"
            "✓ Find the cheapest deals\n"
            "✓ Show premium/best products\n"
            "✓ Browse categories: Electronics, Fashion, Home, Books, Sports, Gaming\n"
            "✓ Place orders with guided checkout\n\n"
            "Try: 'show electronics', 'cheapest', 'best products', or 'search phone'"
        )
        return JsonResponse({'reply': reply})

    # Intent: category browsing
    categories = ['electronics', 'fashion', 'home', 'books', 'sports', 'gaming']
    for cat in categories:
        if cat in text or f'show {cat}' in text or f'{cat} products' in text:
            prods = mongo.get_products(limit=None, category=cat.capitalize())
            if prods:
                reply = f"📦 Here are our {cat.capitalize()} products:\n\n{product_list_reply(prods)}"
            else:
                reply = f"Sorry, no {cat} products available right now."
            return JsonResponse({'reply': reply})

    # Intent: search products
    if any(k in text for k in ['search', 'find', 'show me', 'looking for', 'want']):
        # Extract search term
        search_term = text
        for word in ['search', 'find', 'show me', 'looking for', 'want', 'for']:
            search_term = search_term.replace(word, '').strip()
        
        if search_term:
            prods = mongo.get_products(limit=None, q=search_term)
            if prods:
                reply = f"🔍 Found products matching '{search_term}':\n\n{product_list_reply(prods)}"
            else:
                reply = f"Sorry, couldn't find any products matching '{search_term}'. Try different keywords!"
        else:
            reply = "What would you like to search for? Try: 'search phone' or 'find jeans'"
        return JsonResponse({'reply': reply})

    # Intent: cheapest products
    if 'cheap' in text or 'lowest' in text or 'affordable' in text or 'budget' in text:
        ...
        try:
            ...
            reply = "Sorry, couldn't fetch products right now."
        return JsonResponse({'reply': reply})

    # Intent: best/premium products
    if any(k in text for k in ['best', 'top', 'premium', 'expensive', 'high end', 'luxury']):
        prods = mongo.get_products(limit=None)
        try:
            prods_sorted = sorted(prods, key=lambda x: x.get('price', 0), reverse=True)[:5]
            reply = "⭐ Our premium collection:\n\n" + product_list_reply(prods_sorted)
        except Exception:
            reply = "Sorry, couldn't fetch products right now."
        return JsonResponse({'reply': reply})

    # Intent: all products
    if 'all products' in text or 'show all' in text or 'everything' in text:
        prods = mongo.get_products(limit=10)
        reply = f"📋 Here are some of our products:\n\n{product_list_reply(prods)}\n\nVisit our homepage to see more!"
        return JsonResponse({'reply': reply})

    # Guided order placement
    if 'place order' in text or 'checkout' in text or 'buy for me' in text:
        if not request.user.is_authenticated:
            login_url = request.build_absolute_uri(reverse('store:login'))
            return JsonResponse({'reply': f"🔐 Please log in first to place an order:\n{login_url}"})
        ...
        for key, prompt in fields:
            if not state.get(key):
                ...
                return JsonResponse({'reply': f"📝 {prompt}\n\nAnswer like: '{key.replace('_', ' ')}: <your answer>'"})
        ...
        return JsonResponse({'reply': f"✅ Order placed successfully!\n\nTotal: ₹{order.total}\nPayment: Cash on Delivery\n\nView your order: {order_url}"})
    ...
    reply = (
        ...
        "• 'place order' - Checkout assistance\n"
